...
public class DeleteCommentOnAnswer_ControllerTest {
    @Autowired
    MockMvc mockMvc;
    @Autowired
    ...
    @Test
    @DisplayName("답변에서 댓글 삭제 요청 시 적절한 요청이었다면, 삭제처리 후 200 및 댓글리스트를 반환한다.")
    void updateComment_Answer_success_1() throws Exception {
        ...
        //when 유효한 input
        ResultActions perform = mockMvc.perform(delete("/answers/{answer-id}/comments/{comment-id}", dbAnswer.getId(), dbComment.getId())
                        ...
                        document(
                                ...
                                responseFields(
                                        List.of(
                                                ...
                                                fieldWithPath("[].articleId").type(JsonFieldType.NULL).description("글 식별자입니다"),
