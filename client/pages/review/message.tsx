...
import { selectedTagsAtom, reviewMsgAtom } from '../../atomsHJ';
...
const ReviewMsgContainer = () => {
  const setReviewMsg = useSetRecoilState(reviewMsgAtom);
      ...
      <section className="flex flex-col space-y-10 w-full">
        ...
        <section className="flex w-full p-6 h-[400px] bg-white rounded-[20px] justify-center flex-col space-y-5">
          <ul className="space-y-3 text-main-gray text-xs md:text-base">
            <span>{`예시)`}</span>
            <li>
              ▪ 상세한 답변 감사드려요! 남겨주신 답변을 토대로 문제를 잘 해결할
              수 있었습니다. 감사합니다!
            </li>
            <li>
              ▪ 응원의 말씀 감사드립니다! 조언해주신 내용을 바탕으로 더 발전할
              수 있을 것 같습니다.
            </li>
          </ul>
        ...
        <article className="ml-auto text-right space-x-3">
          {!Object.keys(formData).length || formData.message?.length < 15 ? (
            <div className="text-base sm:text-lg font-bold text-main-gray">
              최소 15자 이상 작성해주세요!
            </div>
          ) : (
            <button className="text-base sm:text-lg font-bold" form="message">
              다음 단계로!
